function dualCrop_G(red_ObjRecon,green_ObjRecon,file_path_red,file_path_green,num,atlas,crop_size,x_shift)
    ...
    %% Make derectories.
        ...
        red_recon_rigid = fullfile(red_recon_path,'rigid');
        ...
        green_recon_rigid = fullfile(green_recon_path,'rigid');
    ...
    %% Rotate the image. 
        ...
        b = green_ObjRecon(:,:,105:110);
        c = green_ObjRecon(:,:,111:115);
        d = green_ObjRecon(:,:,116:end);
        b(b>=100) = 0;
        c(c>=80) = 0;
        d(d>=50) = 0;
        green_ObjRecon(:,:,105:110) = b;
        green_ObjRecon(:,:,111:115) = c;
        green_ObjRecon(:,:,116:end) = d;
        ...
        red_recon_name = fullfile(red_recon_path,['Red',num2str(num),'.nii']);
        % save(red_recon_name,'red_ObjRecon');
        niftiwrite(red_ObjRecon,red_recon_name);
        green_recon_name = fullfile(green_recon_path,['Green',num2str(num),'.nii']);
        % save(green_recon_name,'green_ObjRecon');
        niftiwrite(green_ObjRecon,green_recon_name);
    
    %% Run rigid registration.
    eval(['!bash ./script/rigid.sh ', file_path_green,' ',file_path_red,' ',num2str(num),' ', 0]);

    %% Read the ObjRecon.
    green_ObjRecon = niftiread(fullfile(green_recon_rigid,['Rigid_Green',num2str(num),'.nii']));
    red_ObjRecon = niftiread(fullfile(red_recon_rigid,['Rigid_Red',num2str(num),'.nii']));
    green_ObjRecon(:,:,120:end) = [];
    red_ObjRecon(:,:,120:end) = [];
    ...
    %     if sum(green_BW_ObjRecon,'all') > 10^5
