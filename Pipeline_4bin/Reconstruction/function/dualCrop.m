...
red_ObjRecon = gather(flip(red_ObjRecon,3));
green_ObjRecon = gather(flip(green_ObjRecon,3));
mean_thresh = 10;
...
%% First, rotate the fish to vertical in XY and XZ plane
...
% identify this frame is normal.
if sum(red_BW_ObjRecon,'all') > 10^5
...
%% write the result.
    ...
    RescaledRed_Mip = gather(uint16(RescaledRed_Mip));
    ...
    RescaledGreen_Mip = gather(uint16(RescaledGreen_Mip));
