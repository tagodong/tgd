function [candidate_templates,NR_scores,pixels_thresh] = canTemplateFind(MIPs_path,inter_step,thresh_pixels,isoRm_flag,Red_flag)
    ...
    NR_scores = zeros(size(Red_MIPs,1),1);
    ...
    for i = 1:size(Red_MIPs,1)
        ...
        NR_scores(i) = fmeasure(red_MIP, 'LAPD');
    end
    ...
    inter_NR_scores = reshape(NR_scores(1:step_num*inter_step),inter_step,[]);
    ...
    for i = 1:length(opt_inter_NR_score)
        ...
        [~,index_temp] = min(abs(NR_scores(1+inter_step*(i-1):inter_step*i)-opt_inter_NR_score(i)));
        opt_inter_NR_id(template_num) = inter_step*(i-1)+index_temp;
    end

    %% Select global optimal NR_scores.
    opt_NR_score = prctile(NR_scores,90);
    [~,opt_NR_id] = min(abs(NR_scores-opt_NR_score));
