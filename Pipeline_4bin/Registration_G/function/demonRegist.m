...
%% Run.
    ...
    spmd
        ...
        if start_frame+(spmd_num*spmdIndex-1)*step_size <= end_frame
            G_registDemons(file_path_red,file_path_green,start_frame+spmd_num*(spmdIndex-1)*step_size,step_size,start_frame+(spmd_num*spmdIndex-1)*step_size,num_index,refer_image);
        else
            G_registDemons(file_path_red,file_path_green,start_frame+spmd_num*(spmdIndex-1)*step_size,step_size,end_frame,num_index,refer_image);
        end
    end
    toc;
