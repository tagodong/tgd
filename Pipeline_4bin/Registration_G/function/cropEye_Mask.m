...
%% Run.
    for i = start_frame:step_size:end_frame
        ...
        % read the images.
        red_image = gpuArray(niftiread(fullfile(file_path_red,['regist_red_1_',num2str(ii),'.nii'])));
        green_image = gpuArray(niftiread(fullfile(file_path_green,['regist_green_1_',num2str(ii),'.nii'])));
        Mask = gpuArray(uint16(Mask));
        % run crop.
        red_crop_image = red_image(1:380/2,:,:);
        green_crop_image = green_image(1:380/2,:,:);

        red_crop_image=red_crop_image.*Mask;
        green_crop_image=green_crop_image.*Mask;
        ...
        green_crop_MIP = gather(uint16(green_crop_MIP));
        ...
        green_crop_image = gather(green_crop_image);
