function dual_Crop_big(red_ObjRecon,green_ObjRecon,file_Path_Red,file_Path_Green,num)
    ...
    refer_image = niftiread('/home/user/tgd/Calcium_extract_new/data/Red8225.nii');
    crop_size = [298,487,210];
    % red directory.
...
%% secondly check the fish right vertival, if not flip it.
    ...
    zbb_xy_MIP_flip = max(flip(red_ObjRecon,1),[],3);
    flip_corr = normxcorr2(zbb_xy_MIP,zbb_xy_MIP_flip);
...
%% fourthly crop the background of the 600*600*250 to 400*308*210.
    ...
    CentroID(2) = CentroID(2) - 45;
    CentroID(3) = CentroID(3);
    if CentroID(2) < crop_size(2)/2+5
        CentroID(2) = crop_size(2)/2+5;
    end
    if CentroID(2) > 600-crop_size(2)/2-5
        CentroID(2) = 600-crop_size(2)/2-5;
    end

    if CentroID(1) < crop_size(1)/2+5
        CentroID(1) = crop_size(1)/2+5;
    else
        if CentroID(1) > 600-crop_size(1)/2-5
            CentroID(1) = 600-crop_size(1)/2-5;
        end
    end
    ...
    if CentroID(3) < crop_size(3)/2+3
        ...
        if CentroID(3) > 300-crop_size(3)/2-3
            image_size = [round(CentroID(1)-crop_size(1)/2-4),round(CentroID(1)+crop_size(1)/2+5), ...
                round(CentroID(2)-crop_size(2)/2-4),round(CentroID(2)+crop_size(2)/2+5),...
                300-crop_size(3)-5,300];
            flag = 2;
        else
            image_size = [round(CentroID(1)-crop_size(1)/2-4),round(CentroID(1)+crop_size(1)/2+5), ...
                round(CentroID(2)-crop_size(2)/2-4),round(CentroID(2)+crop_size(2)/2+5),...
