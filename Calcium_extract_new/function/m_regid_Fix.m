function m_regid_Fix(file_Path_Red,file_Path_Green,startFrame,stepSize,endFrame,regid_num,mod)
    ...
    if regid_num == 1
        ...
        %% crop eyes secondly.
            ...
            spmd
                gpuDevice(labindex);
                if startFrame+(spmd_num*labindex-1)*stepSize <= endFrame
                    regid_Fix_3rd(file_Path_Red,file_Path_Green,startFrame+spmd_num*(labindex-1)*stepSize,stepSize,startFrame+(spmd_num*labindex-1)*stepSize,mod);
                else
                    regid_Fix_3rd(file_Path_Red,file_Path_Green,startFrame+spmd_num*(labindex-1)*stepSize,stepSize,endFrame,mod);
                end
            end
            toc;
