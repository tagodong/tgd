...
# data organization parameters # /home/d2/motor_230710-g8s_lssm_7dpf/r20001_24000/regist_red/red_crop/
parser.add_argument('--datadir', default="/media/user/Fish-free11/usman/ali/movingtrain", help='base data directory')
parser.add_argument('--atlas', default="/media/user/Fish-free11/usman/ali/atlas2/atlas.nii.gz", help='atlas filename')
parser.add_argument('--model-dir', default="/home/user/tgd/Voxelmorph/models/", help='model output directory (default: models)')
...
parser.add_argument('--steps-per-epoch', type=int, default=50, help='frequency of model saves (default: 100)')
...
parser.add_argument('--int-steps', type=int, default=14, help='number of integration steps (default: 7)')
parser.add_argument('--int-downsize', type=int, default=2, help='flow downsample factor for integration (default: 2)')
parser.add_argument('--use-probs', default='true', help='enable probabilities')
parser.add_argument('--bidir', default='true', help='enable bidirectional cost function')

# loss hyperparameters
parser.add_argument('--image-loss', default='ncc', help='image reconstruction loss - can be mse or ncc (default: mse)')
...
# load and prepare training data #crop_red_2_*.mat
train_vol_names = glob.glob(os.path.join(args.datadir, '*.nii'))
...
if args.atlas:
    ...
    atlas = vxm.py.utils.load_volfile(args.atlas, np_var='voql', add_batch_axis=True, add_feat_axis=add_feat_axis)
...
# enc_nf = args.enc if args.enc else [16, 32, 64, 128, 256, 512, 512]
# dec_nf = args.dec if args.dec else [512, 256, 128, 128, 64, 32, 32]
enc_nf = args.enc if args.enc else [16, 32, 32, 32]
