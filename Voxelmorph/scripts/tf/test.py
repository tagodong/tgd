...
to the atlas, and computes the dice overlap. Example usage is:
    ...
    test.py \
    --model 0010.pt \
    --atlas /media/jamshaid/Empty/Voxelmorph_data/atlas_image/atlas.nii.gz \
    --scans /media/jamshaid/Empty/Voxelmorph_data/fixed_10_images/green_regist_3_1.nii \
...
parser.add_argument('--gpu', default=0,help='GPU number - if not supplied, CPU is used')
...
registration_model = vxm.networks.VxmDense.load(args.model,device).to(device)
# transform_model = vxm.networks.Transform(inshape, interp_method='nearest').to(device)
# transform_model = vxm.networks.VxmDense(inshape, interp_method='nearest').to(device)
transform_model = vxm.networks.VxmDense(inshape).to(device)
...
for i, scan in enumerate(args.scans):
    ...
    moving_vol = torch.from_numpy(moving_vol).to(device)
    atlas_vol = torch.from_numpy(atlas_vol).to(device)
    # --model 0010.pt --atlas /media/jamshaid/Empty/Voxelmorph_data/atlas_image/atlas.nii.gz --scans  /media/jamshaid/Empty/Voxelmorph_data/fixed_10_images/green_regist_3_14.nii /media/jamshaid/Empty/Voxelmorph_data/moved_10_images/Green14.nii --labels labels.npz

    # predict and apply transform
    warp = registration_model.forward(moving_vol, atlas_vol)

    warped_seg = transform_model.predict([moving_seg, warp]).squeeze().cpu().numpy()
