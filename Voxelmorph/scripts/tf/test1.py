...
parser.add_argument('--atlas', default="/media/jamshaid/Empty/Voxelmorph_data/atlas_image/atlas.nii.gz", help='GPU number - if not supplied, CPU is used')
parser.add_argument('--model', default="0010.pt", help='VxmDense model file')
parser.add_argument('--pairs', default="pairs.txt", help='path to list of image pairs to register')
parser.add_argument('--img-suffix', default="", help='input image file suffix')
...
parser.add_argument('--labels', default="labels.npz", help='optional label list to compute dice for (in npy format)')
...
model = vxm.networks.VxmDense.load(args.model, device).to(device)
# model = vxm.networks.VxmDense.load(args.model, input_model=None)
...
for i in range(len(img_pairs)):
    ...
    fixed_vol = torch.from_numpy(fixed_vol).to(device)
    # predict warp and time
    start = time.time()

    warp = registration_model.forward(moving_vol, fixed_vol)
