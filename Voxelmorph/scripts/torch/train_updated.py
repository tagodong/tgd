...
parser.add_argument('--atlas', default="/media/user/Fish-free11/usman/ali/atlas2/atlas.nii.gz",help='atlas filename (default: data/atlas_norm.npz)')
parser.add_argument('--model-dir', default='/media/Usman/pythonUsman/scripts/torch/models/', help='model output directory (default: models)')
...
parser.add_argument('--steps-per-epoch', type=int, default=10, help='frequency of model saves (default: 100)')
...
parser.add_argument('--enc', type=int, nargs='+', help='list of unet encoder filters (default: 8 16 16 16)')
parser.add_argument('--dec', type=int, nargs='+', help='list of unet decoder filters (default: 16 16 16 16 16 8 8)')
...
enc_nf = [8, 16, 16, 16]  # Use smaller numbers for encoder filters
dec_nf = [16, 16, 16, 16, 16, 8, 8]  # Use smaller numbers for decoder filters
...
for param_name, param in model.named_parameters():
    initial_weights.append((param_name, param.data.clone()))
    print("Initial weight value -", param_name)
    print(param.data)
...
for epoch in range(args.initial_epoch, args.epochs):
    ...
    for step in range(args.steps_per_epoch):
        ...
        log_info = '  '.join((epoch_info, step_info, time_info, loss_info))
        print(log_info, flush=True)

        # write the log info to the log file
        log_file_handle.write(log_info + '\n')
        log_file_handle.flush()

# print updated weight values
updated_weights = []
for param_name, param in model.named_parameters():
    updated_weights.append((param_name, param.data.clone()))
    print("Updated weight value -", param_name)
    print(param.data)
