function demonRegist(file_path_red,file_path_green,red_flag,Red_have,start_frame,step_size,end_frame,num_index,refer_image,gpu_index)
...
%% Run.
    ...
    spmd
        ...
        if start_frame+(spmd_num*spmdIndex-1)*step_size <= end_frame
            registDemons(file_path_red,file_path_green,red_flag,Red_have,start_frame+spmd_num*(spmdIndex-1)*step_size,step_size,start_frame+(spmd_num*spmdIndex-1)*step_size,num_index,refer_image);
        else
            registDemons(file_path_red,file_path_green,red_flag,Red_have,start_frame+spmd_num*(spmdIndex-1)*step_size,step_size,end_frame,num_index,refer_image);
        end
    end
    toc;
