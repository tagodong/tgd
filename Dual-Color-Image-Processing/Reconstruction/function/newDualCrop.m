function newDualCrop(red_ObjRecon,green_ObjRecon,heart_flag,file_path_red,file_path_green,num,atlas,crop_size,x_shift)
    ...
    %% First, rotate the fish to vertical in XY plane
        ...
        %% check if the plane is flip.
            % red_xz_MIP = squeeze(max(red_ObjRecon,[],1));
            % zbb_xz_MIP = squeeze(max(atlas,[],1));
            % cross_corr = normxcorr2(zbb_xz_MIP,red_xz_MIP);
            % atlas = permute(atlas,[2 3 1]);
            % atlas = imrotate(atlas,180,'bicubic', 'crop');
            % atlas = permute(atlas,[3 1 2]);
            % zbb_xz_MIP_flip = squeeze(max(atlas,[],1));
            % flip_corr = normxcorr2(zbb_xz_MIP_flip,red_xz_MIP);
    ...
    %% Write the results.
        ...
        red_crop_MIP_path = fullfile(file_path_red,'Red_Crop_MIP'); %%%%%%%%%% '..','back_up',
