...
do
    ...
    if [ $i -gt 1 ]; then
        ...
        -c [200x200x0,1e-8,10] -f 8x4x2 -s 3x2x1vox -t SyN[0.05,6,0.4] -m CC[$zbb_atlas,$mean_template,1,2] -c [200x200x200x10,1e-7,10] \
    ...
    do
        ...
        antsApplyTransforms -d 3 -v 0 --float 1 -n WelchWindowedSinc -i $path_g/nii/demons_green_3_$j.nii -r $zbb_atlas -o $path_ants_g/ants_g_$j.nii \
        -t ${regist_out_path}/mean2atlas_1Warp.nii.gz -t ${regist_out_path}/mean2atlas_0GenericAffine.mat

        antsApplyTransforms -d 3 -v 0 --float 1 -n WelchWindowedSinc -i $path_r/nii/demons_red_3_$j.nii -r $zbb_atlas -o $path_ants_r/ants_r_$j.nii \
        -t ${regist_out_path}/mean2atlas_1Warp.nii.gz -t ${regist_out_path}/mean2atlas_0GenericAffine.mat
