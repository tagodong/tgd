...
matlab -nodesktop -nosplash -r "file_path = '$Path_r'; red_flag = '$Red_flag'; canTemplateFind_run; quit"

# Run Affine Registration.
bash regist_affine.sh ${Path_g} ${Path_r} ${template}
if [ $? -eq 0 ]; then
	echo "====Build uboot ok!===="
else
...
bash regist_nonrigid.sh ${Path_r}
if [ $? -eq 0 ]; then
	echo "====Build uboot ok!===="
else
	echo "====Build uboot failed!===="
